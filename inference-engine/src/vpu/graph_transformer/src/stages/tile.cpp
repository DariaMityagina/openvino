...
bool getAxisAndTiles(const NodePtr& node, int& axis_par, int& tiles_par) {
...
void FrontEnd::parseTile(const Model& model, const NodePtr& node, const DataVector& inputs, const DataVector& outputs) const {
    ...
    int axis_p = 0, tile_p = 0;
    TileParams params;
    getAxisAndTiles(tile, axis_p, tile_p);
    ...
    printf("params.axis = %d\n\n", int(params.axis));
    printf("params.tiles = %d\n\n", int(params.tiles));
