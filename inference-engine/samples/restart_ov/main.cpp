            ...
            * object detection one **/
            ...
            while (!done) {
                ...
                * before load of the network to the device **/
                ...
                if (run == 5)
                    config[InferenceEngine::MYRIAD_TIMEOUT] = "15";
                else
                    config[InferenceEngine::MYRIAD_TIMEOUT] = "100";
                ...
                * **/
                ...
                // auto duration_nanoseconds = 10 * 1000000000LL * (num_of_tries * 2);

                // while (execTime < duration_nanoseconds) {
                //     // try {
                //     infer_request.Infer(duration_nanoseconds);
                //     // } catch (...) {
                //     //     std::cout << "Timeout from XLink?\n";
                //     //     infer_request.Cancel();
                //     //     continue;
                //     // }
                //     execTime = std::chrono::duration_cast<ns>(Time::now() - startTime).count();
                // }
                // if (execTime >= duration_nanoseconds) {
                //     slog::warn << "Time's up! Let's retry." << slog::endl;
                //     num_of_tries++;
                //     infer_request.Cancel();
                //     continue;
                // }
                ...
                try {
                    ...
                    slog::warn << "Time's up! Let's retry.\n" << slog::endl;
                    slog::warn << "-----------------------\n\n" << slog::endl;
                    num_of_tries++;
                    // infer_request.Cancel();
                    continue;
