...
TEST_P(DynamicBatchedTensorsRunTests, DynamicSetInputRemoteTensorsMultipleInferMultipleDynShapes) {
    // Skip test according to plugin specific disabledTestPatterns() (if any)
    SKIP_IF_CURRENT_TEST_IS_DISABLED()
    ...
    auto batch_shape = Shape{batch, 2, 2, 2};
    auto one_shape_size = ov::shape_size(one_shape);
    auto modelShape = PartialShape{ov::Dimension(1, 10), 2, ov::Dimension(1, 6), 2};
    auto model = BatchedTensorsRunTests::create_n_inputs(2, element::f32, batch_shape, "N...");
